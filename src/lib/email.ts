...
export async function sendReviewPublished(email: string, name: string, firmName: string) {
  ...
  try {
    await resend.emails.send({
      from: FROM,
      to: email,
      subject: `Your review of ${safeFirmName} has been published`,
      html: `
        <h2>Hi ${safeName},</h2>
        <p>Your review of <strong>${safeFirmName}</strong> has been reviewed and published on CampaignGrade.</p>
...
export async function sendReviewRemoved(
  ...
  try {
    await resend.emails.send({
      from: FROM,
      to: email,
      subject: `Your review of ${safeFirmName} has been removed`,
      html: `
        <h2>Hi ${safeName},</h2>
        <p>Your review of <strong>${safeFirmName}</strong> has been removed for the following reason:</p>
        <blockquote>${safeReason}</blockquote>
